...
## 2.2 Holiday Feature Engineering 
...
```{r}
train_holiday_tbl  <- train_data_tbl %>% 
  # full_join Used: 
  # -- Noticed most of the holiday dates are missing from the original retail data 
  full_join(
    country_holiday_tbl %>% 
      select(-year, -country_abbr),
    # Joining by Multiple multiple columns 
    by = c("country"      = "country",
           "invoice_date" = "ds")
    ) %>% 
  # fill missing holiday label 
  # -- label "upwards" & boundary set to weekly 
  # -- Remove missing holiday dates to return back to original data
